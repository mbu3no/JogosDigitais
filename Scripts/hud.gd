...
## Responsavel por: painel principal, intro de fase, fade de transicao e tela de vitoria.
...
var _lbl_rob:      Label
var _lbl_bog:      Label
var _hearts:       Array = []   # 3 Labels de coracao
...
const INTRO_DURATION:  float = 3.0
...
func _build_top_panel() -> void:
	...
	lbl_vidas.position             = Vector2(920, 5)
	lbl_vidas.size                 = Vector2(210, 18)
	...
	for i in range(3):
		var heart := Label.new()
		heart.text                 = "♥"
		heart.position             = Vector2(923 + i * 60, 18)
		heart.size                 = Vector2(60, 40)
	...
	sep2.size     = Vector2(1, 50)
	sep2.position = Vector2(912, 9)
	sep2.color    = Color(0.20, 0.22, 0.35, 0.60)
	add_child(sep2)
...
func _build_intro_overlay() -> void:
	...
	style.bg_color = Color(0.03, 0.04, 0.09, 0.90)
	...
	_intro_lbl_phase.position             = Vector2(0, 162)
	...
	_intro_lbl_name.position             = Vector2(0, 195)
	...
	sep.position = Vector2(326, 292)
	sep.color    = Color(0.25, 0.80, 0.40, 0.35)
	_intro_overlay.add_child(sep)

	# Descricao
	_intro_lbl_desc = Label.new()
	_intro_lbl_desc.position             = Vector2(150, 305)
	_intro_lbl_desc.size                 = Vector2(852, 38)
